formatBucket <- function(eb) {
  ...
  output <- eb %>% 
    ...
    mutate(Descripcion = switch (toString(Cod),
                                 ...
                                 "467" = paste0("Se identificaron ", pad2(num1), " garantía(s) con uno o más Códigos de Deudor (CIS) diferentes a los registrados en la BD01"," correspondiente al periodo de ", periodoEscrito(Periodo), ".(", cutStringError(num1, txt1), ")"),

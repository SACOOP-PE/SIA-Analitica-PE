formatBucket <- function(eb) {
  ...
  output <- eb %>% 
    ...
    mutate(Descripcion = switch (toString(Cod),
                                 ...
                                 "464" = paste0("Se identificaron ", pad2(num1), " crédito(s) en situación contable vencidos con 0 días de atraso en la BD01 correspondiente al periodo de ", periodoEscrito(Periodo), ".(", txt2, ")"),
